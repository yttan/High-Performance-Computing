...
__shared__  dtype scratch[MAX_THREADS];
  ...
  for (int offset = size; (offset+i)<n;offset+=size){
...
volatile dtype *unrolldata = scratch;
    if(n>64) unrolldata[threadIdx.x] += unrolldata[threadIdx.x+32];
    if(n>32) unrolldata[threadIdx.x] += unrolldata[threadIdx.x+16];
    unrolldata[threadIdx.x] += unrolldata[threadIdx.x+8];
    unrolldata[threadIdx.x] += unrolldata[threadIdx.x+4];
    unrolldata[threadIdx.x] += unrolldata[threadIdx.x+2];
    unrolldata[threadIdx.x] += unrolldata[threadIdx.x+1];
